...
def ejecutar_balance_bypass_intermedio(
  ...
  # m_prod_final * brix_obj = m_e2_out * brix_e2_target + m_bp_mix * brix_e1_out
  # m_prod_final = m_e2_out + m_bp_mix
...
def generar_diagrama_detallado(res, pct_bp, pct_e1_agua):
  ...
  dot.edge(
      "SPLITTER",
      "EVAP2",
      label=f" Jugo a Evap 2 ({pct_e2_linea:.0f}%)\n{fmt_lbl('Jugo a Evaporador 2')}",
      color="#2b6cb0",
  )
  dot.edge(
      "EVAP2",
...
figura = generar_diagrama_detallado(
    res, pct_bp_generado, pct_agua_evap_e1
)
